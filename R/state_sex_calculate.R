#' Calculate State Level Sex specific Police Homicide Deaths
#'
#' @description Calculates state level sex specific police homicide counts 
#' and rates from the fatal the encounters data set and population data from
#' the Census Beauru.
#'
#' @usage state_sex_calculate(project_pop = TRUE)
#'
...
state_sex_calculate <- function(project_pop = TRUE){
    pop_df <- state_age_df %>%
    ...
    fe_df_clean %>%
        ...
        group_by(state_abb, State, YEAR, Sex) %>%
        summarise(deaths=n()) %>%
        filter(!is.na(YEAR)) %>%
        ungroup() %>%
        right_join(pop_df, by = c("state_abb", "YEAR", "Sex", "State")) %>%
        ...
        bind_rows(
            fe_df_clean %>%
                ...
                summarise(deaths=n()) %>%
                filter(!is.na(YEAR)) %>%
                filter(Sex == "Missing" | Sex == "Transgender")) %>%
